...
//eliminando repetiçoes no codigo
...
const orderedById = getArrayCopy(characters).sort(
...
const orderedNumbers = getArrayCopy(numbers).sort(
...
const justDisneyMovies = topBrazilmovies
  .filter(({ distributedBy }) => distributedBy === 'Disney')
  .reduce((accumulator, { peopleAmount }) => {
    accumulator += peopleAmount
    return accumulator
  }, 0)
console.log(`${justDisneyMovies} de pessoas assistiram os filmes da Disney`)
...
// const showBrazilMovies = topBrazilmovies
//   .map(({ title }) => `<li>${title}</li>`)
//   .join('')

// ul.innerHTML = showBrazilMovies

// const ShowingBrazilMovies = topBrazilmovies.reduce((accumulator, { title }) => {
//   ul.innerHTML += `<li>${accumulator} ${title}</li>`
//   return accumulator
// }, '')

const ShowingBrazilMovies = topBrazilmovies.reduce(
  (accumulator, { title }) => accumulator + `<li>${title}</li>`,
  ''
)

ul.innerHTML = ShowingBrazilMovies
