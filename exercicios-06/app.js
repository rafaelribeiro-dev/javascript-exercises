...
for (let i = 0; i < fruits.length; i++) {
  if (fruits[i].includes('abacaxi')) {
    console.log(`A string "abacaxi" existe no array  
  fruits.`)
  } else if (fruits[i].includes('pera')) {
...
const currentAge = 35
const ageLimit = currentAge <= 7 && currentAge >= 65
let message = ''
if (ageLimit) {
...
for (let i = 0; i < numbers.length; i++) {
  if ((numbers[i] >= 11) & (numbers[i] <= 90)) {
    selecNumbers.push(numbers[i])
  }
}

console.log(selecNumbers)
...
console.log(
  `O crazyArray tem ${isABoolean} booleans, ${isANumber} números e ${isAString} strings.`
)
