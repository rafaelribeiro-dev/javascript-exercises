...
const beforeThan2000 = tarantinoMovies.filter(movie => movie.release < 2000)

console.log(beforeThan2000)
...
const tvShowsName = tvShows.reduce((acc, { name }) => {
  return `${acc} ${name} \n`
}, [])
...
const listOfGames = cart.reduce((acc, { name }) => {
  return `${acc} - ${name} \n`
}, [])
