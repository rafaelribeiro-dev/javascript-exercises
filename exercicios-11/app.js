...
let car = {
  name: 'FIT',
  brand: 'Honda',
  colors: ['Preto', 'Prata', 'Azul Metálico'],
  isRunning: false,
  run() {
    this.isRunning = true
  ...
  getColorsMessage() {
    const colorsAvailable = this.colors
      .join(', ')
      .replace(
        `, ${this.colors[this.colors.length - 1]}`,
        ` e ${this.colors[this.colors.length - 1]}`
      )

    return `O ${this.name} está disponível nas cores ${colorsAvailable}`
  }
}
/*
  04
