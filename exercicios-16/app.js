...
const eggArea = document.querySelector('.egg')
...
const showCoordinates = ({ offsetX, offsetY }) => {
  eggArea.textContent = `Eixo X: ${offsetX} | Eixo Y: ${offsetY}`
}
eggArea.addEventListener('mousemove', showCoordinates)
...
const changeEggColor = () => {
  eggArea.style.backgroundColor = 'lightgoldenrodyellow'
}
...
const containFrontendDeveloper = people.some(
  ({ profession }) => profession === 'Front-end developer'
)

if (containFrontendDeveloper) {
