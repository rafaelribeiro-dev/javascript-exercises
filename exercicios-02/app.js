...
const initials =
  mozilla.slice(0, 1) + mozilla.slice(8, 9) + mozilla.slice(18, 19)
...
*/
let caractersAmount = 0
for (let i = 0; i < programmingLanguage.length; i++) {
  caractersAmount += programmingLanguage[i].length
}
console.log(caractersAmount)
...
console.log(`A última letra da palavra é: ${lastLetter}`)
...
newName = `${names[0].toUpperCase() + names.slice(1, 6)} ${
  names[7].toUpperCase() + names.slice(8)
}`
